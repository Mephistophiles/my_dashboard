...
use colored::*;
use crate::weather::{WeatherService, WeatherForecast, analyze_weather_for_photography, WeatherAnalysis};
use crate::solar::{SolarService, AuroraForecast};
use crate::golden_hour::{GoldenHourService, GoldenHourInfo};
use crate::photography_tips::{PhotographyTipsService, PhotographyTips};
...
impl PhotographyDashboard {
    ...
    pub async fn generate_dashboard(&self) -> Result<DashboardSummary, Box<dyn std::error::Error>> {
        ...
        let aurora_forecast = self.solar_service.predict_aurora(&solar_wind_data, &geomagnetic_data);
        
        // Получаем информацию о золотом часе
        let golden_hour_info = self.golden_hour_service.calculate_golden_hours(current_time);
    ...
    fn is_golden_hour_today(&self, golden_hour_info: &GoldenHourInfo, current_time: DateTime<Local>) -> bool {
    ...
    pub fn print_dashboard(&self, summary: &DashboardSummary) {
        println!("\n{}", "=".repeat(60));
        println!("{}", "📸 ДАШБОРД ДЛЯ ФОТОГРАФОВ 📸".bold().white().on_blue());
        println!("{}", "=".repeat(60));
        ...
        println!("\n{}: {:.1}/10", "Оценка погоды".bold(), summary.weather_score);
        println!("{}: {:.1}%", "Вероятность северных сияний".bold(), summary.aurora_probability * 100.0);
        println!("{}: {}", "Золотой час сегодня".bold(), if summary.is_golden_hour_today { "Да" } else { "Нет" });
