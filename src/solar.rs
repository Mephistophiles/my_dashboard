...
impl SolarService {
    ...
    pub fn predict_aurora(&self, solar_wind: &SolarWindData, geomagnetic: &GeomagneticData) -> AuroraForecast {
        ...
        if forecast.visibility_probability > 0.7 {
            forecast.recommendations.push("Отличные условия для наблюдения северных сияний!".to_string());
            forecast.recommendations.push("Ищите темные места вдали от городских огней".to_string());
        } else if forecast.visibility_probability > 0.4 {
            forecast.recommendations.push("Умеренная вероятность северных сияний".to_string());
        } else {
            forecast.recommendations.push("Низкая вероятность северных сияний".to_string());
        }

        // Проблемы
        if geomagnetic.kp_index > 6.0 {
            forecast.concerns.push("Высокая геомагнитная активность может повлиять на электронику".to_string());
        }
...
pub fn print_aurora_forecast(forecast: &AuroraForecast) {
    ...
    println!("\n{}: {:.1}%", "Вероятность видимости".bold(), forecast.visibility_probability * 100.0);
